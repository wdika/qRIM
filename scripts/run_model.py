...
def run_rim(args, model, data_loader):
    ...
    for i, data in enumerate(data_loader):
        ...
        plt.axis('off')
        plt.subplot(2, 4, 5)
        plt.imshow(torch.abs(R2star_map_target.squeeze()).detach().cpu().numpy(), cmap='gray')
        plt.title('R2* target')
        plt.axis('off')
        plt.subplot(2, 4, 6)
        plt.imshow(torch.abs(S0_map_target.squeeze()).detach().cpu().numpy(), cmap='gray')
        plt.title('S0 target')
        plt.axis('off')
        plt.subplot(2, 4, 7)
        plt.imshow(torch.abs(B0_map_target.squeeze()).detach().cpu().numpy(), cmap='gray')
        plt.title('B0 target')
        plt.axis('off')
        plt.subplot(2, 4, 8)
        plt.imshow(torch.abs(phi_map_target.squeeze()).detach().cpu().numpy(), cmap='gray')
        plt.title('phi target')
        plt.axis('off')
        # plt.subplot(3, 4, 9)
