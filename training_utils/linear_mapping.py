...
class LeastSquares:
    ...
    def lstq_pinv(self, A, Y, lamb=0.0):
        ...
        if Y.dim() == 2:
            ...
            return torch.matmul(torch.matmul(torch.inverse(torch.matmul(Y.permute(0, 2, 1), Y)), Y.permute(0, 2, 1)),
                                A)
...
def B0_phi_mapping(image, TEs, mask_brain, mask_head, fullysample):
    ...
    for j in range(sz_phase[1]):  # loop over batch size
        ...
        for i in range(phase_sample.shape[0]):
            phase_unwrapped[i] = torch.from_numpy(unwrap_phase(
                np.ma.array(phase_sample[i].squeeze().cpu().detach().numpy(), mask=mask_head_np)).data).float().to(
                image.device)
        phase_diff_set = []
        TE_diff = []
...
class signal_model_forward(object):
    def __call__(self, R2star_map, S0_map, B0_map, phi_map, mask_brain, TEs=torch.Tensor([3.0, 11.5, 20.0, 28.5]),
                 sequence='MEGRE'):
...
def signal_model_MEGRE(R2star_map, S0_map, B0_map, phi_map, mask_brain, TEs):
    ...
    S0_map_real = S0_map
    S0_map_imag = phi_map
    images = torch.stack([torch.stack((S0_map_real * torch.exp(-TEs[i] * scaling * R2star_map) * torch.cos(
        B0_map * scaling * -TEs[i]) - S0_map_imag * torch.exp(-TEs[i] * scaling * R2star_map) * torch.sin(
        B0_map * scaling * -TEs[i]),
                                       S0_map_real * torch.exp(-TEs[i] * scaling * R2star_map) * torch.sin(
                                           B0_map * scaling * -TEs[i]) + S0_map_imag * torch.exp(
                                           -TEs[i] * scaling * R2star_map) * torch.cos(B0_map * scaling * -TEs[i])),
                                      -1)
                          for i in range(TEs.size(0))], 1)
