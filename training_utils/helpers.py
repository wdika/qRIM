...
def loss_est_analytic(grad_fun, TEs, sense_k, eta_y, y_ksp, mask_subsampling, mask_brain):
    scaling = 1.0e-3
    nr_TEs = len(TEs)
    eta_img = grad_fun(eta_y[0].unsqueeze(0), eta_y[1].unsqueeze(0), eta_y[2].unsqueeze(0), eta_y[3].unsqueeze(0),
                       mask_brain)
    ...
    S0_part_der = torch.stack(
        [torch.stack((torch.exp(-TEs[i] * scaling * R2star_map) * torch.cos(B0_map * scaling * -TEs[i]),
                      -torch.exp(-TEs[i] * scaling * R2star_map) * torch.sin(B0_map * scaling * -TEs[i])), -1)
         for i in range(nr_TEs)], 1)

    R2str_part_der = torch.stack([torch.stack((-TEs[i] * scaling * torch.exp(-TEs[i] * scaling * R2star_map) * (
                S0_map_real * torch.cos(B0_map * scaling * -TEs[i]) - S0_map_imag * torch.sin(
            B0_map * scaling * -TEs[i])),
                                               -TEs[i] * scaling * torch.exp(-TEs[i] * scaling * R2star_map) * (
                                                           -S0_map_real * torch.sin(
                                                       B0_map * scaling * -TEs[i]) - S0_map_imag * torch.cos(
                                                       B0_map * scaling * -TEs[i]))), -1)
                                  for i in range(nr_TEs)], 1)

    S0_map_real_grad = diff_data_inverse[..., 0] * S0_part_der[..., 0] - diff_data_inverse[..., 1] * S0_part_der[
        ..., 1]
    S0_map_imag_grad = diff_data_inverse[..., 0] * S0_part_der[..., 1] + diff_data_inverse[..., 1] * S0_part_der[
        ..., 0]
    R2star_map_real_grad = diff_data_inverse[..., 0] * R2str_part_der[..., 0] - diff_data_inverse[..., 1] * \
                           R2str_part_der[..., 1]
    R2star_map_imag_grad = diff_data_inverse[..., 0] * R2str_part_der[..., 1] + diff_data_inverse[..., 1] * \
                           R2str_part_der[..., 0]
...
def image_loss(image, target, mask_brain, args):
    loss_selector = {'l1': lambda x, t: F.l1_loss(x, t, reduction='none'),
                     ...
                     'ssim': lambda x, t, m, img_reshape: -ssim.ssim_uniform(img_reshape(x * m), img_reshape(t * m),
                                                                             window_size=7, reduction='none'),
                     }
