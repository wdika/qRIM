...
def B0mapping(coilimgs, sense, mask_brain):
    ...
    for i in range(0, phase_unwrapped.shape[0] - TEnotused):
        ...
        phase_diff_set[i] = phase_diff_set[i] - np.round(
            np.sum(phase_diff_set[i] * mask_brain.flatten()) / np.sum(mask_brain.flatten()) / 2 / np.pi) * 2 * np.pi
    ...
    scaling = 1e-3
    ls = LeastSquares()
    B0_map_tmp = ls.lstq_pinv(torch.from_numpy(np.transpose(np.expand_dims(phase_diff_set, 2), (1, 0, 2))),
        ...
        if coilimgs != False:
            ...
            for dim in range(3):
                ...
                for itr_dim in range(round(size_dim / 2) - half_nr_of_slices, round(size_dim / 2) + half_nr_of_slices):
                    filename_save = os.path.join(savepath, folder_subject, planes[dim],
                                                 'Subcortex_' + str(subjectID).zfill(4) + '_' + planes[
                                                     dim] + '_' + str(itr_dim) + '.h5')
